...
class PersonItem(HubItemBase):
    """
    An object representing a club member's particulars. The object self-sets
    its own GUID and timestamp upon instantiation. The setters are private. The
    object can be round-tripped to and from JSON using PersonDto.
    ...
    def __hash__(self):
        ...
        )  # hash based on all attributes, including private attributes, but excluding comment

    def __eq__(self, other: object) -> bool: # ignore comment field

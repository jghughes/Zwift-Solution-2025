...
def test01():
    ...
    print(f"Starting: head-to-head benchmarking of serial-processing versus parallel-processing with {len(riders)} riders, {len(PULL_DURATION_OPTIONS_SEC)} pull periods, and consequently {pretty_number_of_sequences_before_pruning} paceline_rotation sequences (no solution-space pruning. all sequences evaluated).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        all_conceivable_paceline_rotation_sequences = generate_all_paceline_rotation_sequences_in_the_total_solution_space(len(riders), PULL_DURATION_OPTIONS_SEC)

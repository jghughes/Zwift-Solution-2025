...
def main():
    ...
    # PREQUEL: CALCULATE SOME BENCHMARKS AND UPPER/LOWER BOUNDS
    ...
    _, plan_line_items, halted_rider = make_a_pull_plan_complying_with_exertion_constraints(riders, simplest_pull_durations, lowest_bound_speed_as_array, 0.80)
    
    plan_line_items_displayobjects = populate_pullplan_displayobjects(plan_line_items)
    log_concise_pullplan_displayobjects(f"\nSIMPLE PULL-PLAN: {round(plan_line_items[halted_rider].speed_kph,1)} kph (one-minute pulls for everybody. IF capped at 80%.)", plan_line_items_displayobjects, logger)

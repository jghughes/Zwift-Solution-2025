...
from jgh_formulae08 import make_a_pull_plan_complying_with_exertion_constraints, search_for_optimal_pull_plans_concurrently
...
def main():
    ...
    logger.info(f"Upper bound pull        :  {round(r01_speed)} kph @ {round(r01.get_permitted_30sec_pull_watts())} W ({round(r01.get_permitted_30sec_pull_watts()/r01.weight_kg, 1)} W/kg) by {r01.name} for a pull of {round(r01_duration)} seconds.")
    ...
    logger.info(f"Lower bound pull        :  {round(r01_speed)} kph @ {round(r01.get_permitted_4_minute_pull_watts())} W ({round(r01.get_permitted_4_minute_pull_watts()/r01.weight_kg)} W/kg) by {r01.name} for a pull of {round(r01_duration)} seconds.")
    ...
    (pull_plans, total_alternatives, total_iterations, compute_time) = search_for_optimal_pull_plans_concurrently(riders, STANDARD_PULL_PERIODS_SEC, lowest_bound_speed, MAX_INTENSITY_FACTOR)

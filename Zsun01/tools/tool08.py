"""
This monster tool is not used directly in the Brute production pipeline.
I wrote this tool to consolidate power data to unearth correlations
between raw and synthetic power data and Zwift's inscrutable zFTP metric. 
Go to tool12.py to learn all about how I used the data generated by this tool 
to experiment extensively with linear regression using sklearn, scipy, and matplotlib.
...
DaveK from all sources, namely Zwift, ZwiftPower, and ZwiftRacingApp. It
performs automated power curve modeling and dataset generation for the
entire set of Zwift riders using their 90-day best power data. There
are approx 1,500 riders in the club, and this tool processes
all of them in one go. It takes a few minutes to run. After filtering
out riders who are deemed to not have data suitable for modeling, most
likely because they don't race enough or because there is some or other
flaw in their profiles, the number reduces to around 260. These riders 
have high-fidelity models that are suitable for regression analysis. 
The data generated by this tool is consolidated in the special purpose
RegressionModellingItem for each rider.

Note that this is the earliest tool in which we see the heap powerful
ScrapedZwiftDataRepository class in action. It's .populate() method 
is used in the production pipeline in Brute to load all the Zwift 
...
of ZsunRiderItems to be copied manually in the data folder of Zsun01
in production. The repository has been extensively debugged.

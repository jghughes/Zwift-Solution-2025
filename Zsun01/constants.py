...
DEFAULT_EXERTION_INTENSITY_FACTOR_LIMIT = 1.0 # each team has its own factor. see class RepositoryOfTeams in Zsun01/src/data_repositories/team_rosters.py for details. This is the default factor used when a team does not have a specific factor defined. It represents the standard exertion intensity factor for teams that do not have a specific factor defined. The value is set to 1.0, meaning no adjustment to the exertion intensity is applied. This is a placeholder value and can be adjusted as needed.

...
ROTATION_SEQUENCE_UNIVERSE_SIZE_PRUNING_GOAL = 1000 # Somewhat arbitary. Emprically detemined. See main01() in formula08.py for details of the determination. The sweet zone is 1,000 - 2,500, which keeps compute time within 7 - 14s zone for up to 6 riders. A threshold for the number of alternative solutions we are willing to analyse by uncompromising brute force before we lose patience with the amount of time taken to compute. More than this and our strategy to limt the time is to apply a heuristic to prune the solution space. It is preferable to minimise the amount of pruning because pruning is a heuristic and all heuristics entail risk (albeit tiny in this case) that they will inadvertently exclude a winning solution. The Cartesian cross product of eight riders and seven pull sequences generates a solution space of 5.76 million which would take literally days to compute. By pruning, we prune these very nearly down to the ROTATION_SEQUENCE_UNIVERSE_SIZE_PRUNING_GOAL. 

...
AERO_POSITION_FACTOR_DEFAULT = AERO_POSITION_FACTOR_HOODS # for Brute, this seems to produce the most sensible results that pass the smell-test.
# AERO_POSITION_FACTOR_DEFAULT = AERO_POSITION_FACTOR_TT # no idea if this is better or worse for iTT since we do not have any benchmarks.
...
PULL_DURATION_OPTIONS_SEC: tuple[float] = [0.0, 30.0, 60.0, 120.0, 180.0, 240.0] # NB. the elements MUST BE IN ASCENDING ORDER otherwise the algorithms will not work correctly. The list can be truncated to reduce compute time,maybe to handle larger groups of riders for example, but THE VALUES OF THE ELEMENTS ARE FIXED, they MUST NOT BE CHANGED otherwise the algorithms will generate nonesense. The values in this array map to the code in RiderComputeItem.get_proxy_pull_watts(..), which in turn maps to the values of RiderComputeItem.get_proxy_30sec_pull_watts(), RiderComputeItem.get_proxy_1_minute_pull_watts(), RiderComputeItem.get_proxy_2_minute_pull_watts(), RiderComputeItem.get_proxy_3_minute_pull_watts(), and RiderComputeItem.get_proxy_4_minute_pull_watts(). These methods are where the magic of curve-fitting comes together with the empirical experience of DaveK as a regular TTT racer. Each method is based parameters for what DaveK feels he can achieve in terms of repeated efforts and over/under intervals in a race. Assuming 8 riders, I recommend max 6 pull periods, otherwise the solution space becomes too large and the algorithm takes too long to compute (more than a minute). The pull periods are in seconds, and they represent the time each rider spends at the front of the paceline during a ride. The first_name element (0.0) is included to represent the case where a rider does not take a pull. The functions affected by PULL_DURATION_OPTIONS_SEC produce the Cartesian product of the allowed pull periods for each rider. For n riders and k allowed pull periods, it generates k^n possible sequences. Each row in the returned array is a sequence of pull periods for the paceline. For instance, 6 pull periods and eight riders generates 6^8 = 1,679,616 possible sequences. This is a large number, but it is manageable for the algorithm to process within a reasonable time frame, especially with the solution-space pruning applied prior to compute expensive processing. Pruning itself is compute intense, but much less intense than subsequent processing.

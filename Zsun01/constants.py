...
COEFFICIENT_g: float = 9.80665  # gravity (m/s^2)
...
INITIAL_VELOCITY_GUESS_FOR_NEWTON_SOLVER_KPH = 55 # arbitrary best-guess (on the high side as recommended) of 55 km/h for riders in this domain for Newton-Raphson methods to solve speed from power in jgh_formulae00.solve_velocity_from_power(power: float, Cd: float, A: float, Crr: float, total_mass: float, slope: float) -> float: and to solve time taken to complete a specified distance jgh_formulae00.calculate_seconds_riding_solo(rider: RiderComputeItem, distanceKm: float, slope: float = 0.0) -> float: .

REQUIRED_NEWTON_SOLVER_VELOCITY_PRECISION_KPH = 0.01 # The convergence tolerance for the Newton-Raphson root-finding solver used in jgh_formulae00.solve_velocity_from_power(...) above. 

REQUIRED_NEWTON_SOLVER_DISTANCE_PRECISION_KM = 1e-3 # The convergence tolerance for the Newton-Raphson root-finding solver used in jgh_formulae00.calculate_seconds_riding_solo(...). The solver finds the duration (seconds) at which the distance residual (km) is zero. This value specifies the acceptable residual error in kilometres. It is not derived from any specific mathematical or domain-specific principle, but rather serves as a practical threshold for numerical convergence. 1e-3 km = 1 meter, which is more than sufficient precision for cycling distance calculations.

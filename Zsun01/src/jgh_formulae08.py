...
def main() -> None:
    ...
    pull_duration_scenario = [30.0, 20.0, 10.0, 5.0]
    ...
    for i, pull_speed_scenario in enumerate(pull_speed_scenarios):
        ...
        # # Generate dict of riders with segmented data rider power datapoints for one hour
        # segmented_data : Dict[ZwiftRiderItem, Dict[int, float]] = {}
        # for rider in riders:
        #     segmented_data[rider] = segment_data(rider_power_datapoints[rider], segment_durations)

        # # log the segmented data
        # for rider, data in segmented_data.items():
        #     logger.info(f"\n{rider.name}")
        #     for duration, avg_power in data.items():
        #         logger.info(f"{duration}sec: {avg_power:.2f}W")

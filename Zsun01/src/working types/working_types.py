...
class CurveFittingResultItem:
    ...
    sixty_min_curve_coefficient   : float = 0.0   # Coefficient for FTP modeling
    sixty_min_curve_exponent      : float = 0.0   # Exponent for FTP modeling
    sixty_min_curve_r_squared     : float = 0.0   # R-squared value for FTP curve fitting
...
class PacelineIngredientsItem:
    riders_list                  : List[RiderBruteItem]      = field(default_factory=list)
...
class PacelineComputationReportItem:
    ...
    rider_contributions                          : Dict[RiderBruteItem, RiderContributionItem] = field(default_factory=lambda: defaultdict(RiderContributionItem))
...
class PackageOfPacelineComputationReportItem:
    ...
    dict_of_solutions                     : Dict[PacelinePlanTypeEnum, PacelineComputationReportItem] = field(default_factory=lambda: defaultdict(PacelineComputationReportItem))

...
from constants import ROTATION_SEQUENCE_UNIVERSE_SIZE_PRUNING_GOAL, PULL_DURATION_OPTIONS_SEC
from jgh_formatting import format_number_with_comma_separators
from jgh_formulae03 import generate_all_paceline_rotation_sequences_in_the_total_solution_space, prune_all_sequences_of_pull_periods_in_the_total_solution_space
...
def test01():
    ...
    paceline_ingredients = PacelineIngredientsItem(
        riders_list                   = _riders,
        sequence_of_pull_periods_sec  = PULL_DURATION_OPTIONS_SEC,
        pull_speeds_kph               = [30.0] * len(_riders),
        max_exertion_intensity_factor = 0.95
    )

    save_filename_without_ext01 = f"{_output_filename_01_stub}{len(_riders)}"

    print(f"Starting: head-to-head benchmarking of serial-processing versus parallel-processing with {len(_riders)} riders, {len(PULL_DURATION_OPTIONS_SEC)} pull periods, and consequently {_pretty_number_of_sequences_before_pruning} paceline_rotation sequences (no solution-space pruning. all sequences evaluated).")
    ...
    _ = generate_paceline_solutions_using_serial_processing_algorithm(paceline_ingredients, _all_conceivable_paceline_rotation_sequences)
    ...
    _ = generate_paceline_solutions_using_parallel_workstealing_algorithm(paceline_ingredients, _all_conceivable_paceline_rotation_sequences)
    ...
    report_lines.append(f"Number of riders: {len(_riders)}\n\n")
    report_lines.append(f"Number of standard pull periods: {len(PULL_DURATION_OPTIONS_SEC)}\n\n")
    report_lines.append(f"Consequential number of paceline-rotation sequences: {_pretty_number_of_sequences_before_pruning}\n\n")
    ...
    plt.title(f"Compute Time: Serial-processing vs Parallel-processing (work stealing): Paceline rotation sequences: {_pretty_number_of_sequences_before_pruning}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        _dict_of_all_riders = read_rider_compute_dict_from_json(Path( DIRPATH_VISUAL_STUDIO_PYTHON_PROJECT), FILENAME_RIDER_COMPUTE_DTO_JSON_DICT)
        _riderIDs = RepositoryOfTeamRosters.get_IDs_of_riders_on_a_team("scratchpad")
        _riders: List[RiderComputeItem] = lookup_Items_by_ZwiftID(_riderIDs, _dict_of_all_riders, RiderComputeItem)
        _all_conceivable_paceline_rotation_sequences = generate_all_paceline_rotation_sequences_in_the_total_solution_space(len(_riders), PULL_DURATION_OPTIONS_SEC)
        _pretty_number_of_sequences_before_pruning = format_number_with_comma_separators(len(_all_conceivable_paceline_rotation_sequences))

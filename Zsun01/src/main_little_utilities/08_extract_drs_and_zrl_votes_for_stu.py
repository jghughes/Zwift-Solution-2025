...
ABBREVIATED_COLUMN_HEADINGS: Dict[str, str] = {
    ...
    "DRS Captain": "DRS Captain",
    "Assigned DRS Team": "DRS Team",
    "DRS Start Time": "DRS Start",
    "Captain / Co-Captain": "Captain",
    "Assigned ZRL Team": "ZRL Team",
    "ZRL Start Time / League": "ZRL Start",
    "Notes": "Notes"
}
...
def make_dataframe_of_yes_votes_in_america(input_dataframe: pd.DataFrame, vote_columns: List[str], columns_to_keep: List[str]) -> pd.DataFrame:
    ...
    """
    if not vote_columns:
        return input_dataframe[columns_to_keep].copy()
    ...
    input_dataframe = input_dataframe.loc[is_yes_vote_mask, columns_to_keep].copy()
    return input_dataframe

def make_dataframe_columns_to_keep(input_dataframe: pd.DataFrame, columns_to_keep: List[str]) -> pd.DataFrame:
    """
    Creates a dataframe containing only the specified columns.
    """
    return input_dataframe[columns_to_keep].copy()
...
def abbreviate_column_headings(df: pd.DataFrame, abbreviated_column_headings: dict) -> pd.DataFrame:
    if not abbreviated_column_headings:
        return df
...
if __name__ == "__main__":
    ...
    try:
        ...
        _csv_filename_computed_by_kgh = "ZSUNR_Fall_2026_vote_assignments_for_Stu.csv"

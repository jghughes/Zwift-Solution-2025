...
def first_rider_is_strongest(strongest_rider : ZsunItem, result : PackageOfPacelineComputationReportItem, desired_solution_index : int ) -> bool:
...
def get_solution_speed(optimal_result: PackageOfPacelineComputationReportItem, desired_solution_index : int) -> float:
...
def get_hardest_intensity(optimal_result: PackageOfPacelineComputationReportItem, desired_solution_index : int) -> float:
    if not optimal_result.solutions:
        return 0
    solution = optimal_result.solutions[desired_solution_index]
...
def get_intensity_suffered_by_weakest_rider(optimal_result: PackageOfPacelineComputationReportItem, desired_solution_index : int) -> float:
    if not optimal_result.solutions:
        return 0
    solution = optimal_result.solutions[desired_solution_index]
...
def main():
    ...
    def first_and_last_rider_filter(result: PackageOfPacelineComputationReportItem, desired_solution_index: int) -> bool:
        if not result.solutions or not result.solutions[desired_solution_index].rider_contributions:
            return False

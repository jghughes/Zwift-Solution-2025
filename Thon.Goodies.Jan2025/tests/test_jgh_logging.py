...
from jgh_loggingV2 import jgh_configure_logging
from jgh_file_finder import find_dirpath

class TestConfigureLogger(unittest.TestCase):

    def test_configure_logger_with_valid_settingsfile_loggingsection(self) -> None:
        jgh_configure_logging('appsettingsValid.json')
    ...
    # @patch('jgh_logging.find_dirpath')
    ...
    #     jgh_configure_logging('appsettings.json') # This test is expecting a settings file specifying a console handler and a file handler.
    #     logger = logging.getLogger()
    #     self.assertTrue(logger.hasHandlers())
    #     self.assertEqual(len(logger.handlers), 2)
    #     console_handler = next((h for h in logger.handlers if h.name == 'consoleHandler'), None)

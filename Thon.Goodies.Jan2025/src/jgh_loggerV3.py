...
def configure_loggerV3(appsettings_filename: Optional[str] = None)-> None:

    try:

        logger = logging.getLogger() # Get the root logger, we will add handlers to this logger
        logger.setLevel(logging.DEBUG) # Set the default logging level to DEBUG, we override this in the handlers if needed
...
if __name__ == "__main__":
    ...
    configure_loggerV3(appsettings_filename)

...
class JghListDictionary(Generic[TKey, TValue]):
    ...
    def remove_key(self, key: TKey) -> List[TValue]:
        ...
        If the key doesn't exist, the remove_key method will return an empty list,
        indicating that no key was removed. This ensures that the method
        accurately reflects whether the specified key was present and removed
        from the dictionary.
    ...
    def remove_value_from_key(self, key: TKey, value: TValue) -> int:
        """
        Removes all occurrences of the value from the list associated with 
        the given key.

        If the specified value does not exist in the list associated with a key,
        the remove_value_from_key method will leave the list unchanged.
        This ensures that the method only removes existing values and does not
        affect the list if the value is not present.
        ...
        self.backingstore_dict[key] = list(filter(lambda v: v != value, self.backingstore_dict[key]))

        removed_count = original_length - len(self.backingstore_dict[key])
    ...
    def get_selected_values_from_key(self, key: TKey, value_selector: Callable[[TValue], bool]) -> List[TValue]:
        """
        Retrieves all values associated with the given key that match the given selector.
...
if __name__ == "__main__":
    ...
    removed_count, keys_with_removed_value = dictionary.remove_value_from_all_keys(value_to_count)
    print(f"Number of occurrences of value {value_to_count} removed:", removed_count)
    print(f"Keys from which value {value_to_count} was removed:", keys_with_removed_value)
    ...
    print("Vegetables after clearing values:", dictionary.get_values_belonging_to_key("vegetables"))
